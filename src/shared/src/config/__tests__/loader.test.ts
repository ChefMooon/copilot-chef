...
`;
  ...
  it("uses defaults when no config file exists", () => {
    process.env.COPILOT_CHEF_DATABASE_URL = "file:./fallback.db";
    const config = loadServerConfig(path.join(tmpDir, "nonexistent.toml"));

    expect(config.server.port).toBe(3001);
    expect(config.server.host).toBe("127.0.0.1");
    expect(config.server.logLevel).toBe("info");
    expect(config.database.url).toBe("file:./fallback.db");
...
`;
  ...
  it("falls back to bare DATABASE_URL for backward compat", () => {
    process.env.DATABASE_URL = "file:./compat.db";
    const config = loadServerConfig(path.join(tmpDir, "nonexistent.toml"));

    expect(config.database.url).toBe("file:./compat.db");
  });

  it("prefers COPILOT_CHEF_DATABASE_URL over bare DATABASE_URL", () => {
    process.env.DATABASE_URL = "file:./bare.db";
  ...
  it("throws on invalid config (missing required database.url)", () => {
    expect(() =>
      loadServerConfig(path.join(tmpDir, "nonexistent.toml"))
    ).toThrow();
  });
