...
export function buildSystemPrompt(context: SystemPromptContext = {}): string {
  ...
  return [
    ...
    ``,
    `## Available Tools`,
    `- create_meal: Create a meal entry in the calendar. Required fields: name, mealType, date. Optional fields: notes, ingredients.`,
    `- list_meals: Retrieve meals in context, optionally with from/to ISO dates.`,
    ``,
    `## Tool Usage Rules`,
    `- When creating meals, always use field names exactly: name, mealType, date, notes, ingredients.`,
    `- Never use alternate field names like title, type, or ingredientsJson when calling tools.`,
    `- Ask for clarification if date or meal type is ambiguous.`,
    ``,
    `## Response Style`,
