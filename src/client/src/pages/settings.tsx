...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      setConnectionDraft({
        serverUrl: cached.connection.serverUrl,
        apiKey: cached.connection.apiKey,
        autoLaunchServer: cached.connection.autoLaunchServer,
      });
      return;
    }
    ...
    loadClientConfig()
      .then((config) => {
        setConnectionDraft({
          serverUrl: config.connection.serverUrl,
          apiKey: config.connection.apiKey,
          autoLaunchServer: config.connection.autoLaunchServer,
        });
      })
      .catch(() => {
  ...
  const handleSaveConnection = async () => {
    ...
    try {
      ...
      const next: ClientConfig = {
        connection: {
          serverUrl: connectionDraft.serverUrl,
          apiKey: connectionDraft.apiKey,
          autoLaunchServer: connectionDraft.autoLaunchServer,
          serverBinaryPath: current?.connection.serverBinaryPath ?? "",
        },
        updates: current?.updates ?? { checkOnStartup: true },
        ui: current?.ui ?? { theme: "system" },
      };
      ...
      <CollapsibleSection id="connection" label="Connection">
        <div className={styles.card}>
          ...
          <div className={styles.toggleList} style={{ marginTop: "1rem" }}>
            <ToggleRow
              checked={connectionDraft.autoLaunchServer}
              label="Auto-launch server"
              description="Automatically start the Copilot Chef server when the app opens."
              onChange={(checked) =>
                setConnectionDraft((prev) => ({
                  ...prev,
                  autoLaunchServer: checked,
                }))
              }
            />
          </div>
