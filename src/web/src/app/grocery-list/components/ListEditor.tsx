...
import { listProgress, type GroceryItem, type GroceryList, formatListDate } from "@/lib/grocery";
...
type Props = {
  ...
  onUpdateItem: (listId: string, itemId: string, updates: Partial<GroceryItem>) => Promise<void>;
  onDeleteItem: (listId: string, itemId: string) => Promise<void>;
  onReorder: (listId: string, itemIds: string[]) => Promise<void>;
  onShop: () => void;
};

export function ListEditor({ list, onUpdateList, onDeleteList, onCreateItem, onUpdateItem, onDeleteItem, onReorder, onShop }: Props) {
  ...
  const [dragOverState, setDragOverState] = useState<{ itemId: string; position: "before" | "after" } | null>(null);
  ...
  const dropItem = async (targetItemId: string, position: "before" | "after") => {
    ...
    <div className={styles.editorPanel}>
      <div className={styles.editorHeader}>
        <div className={styles.editorTitleRow}>
          {editingName ? (
            ...
            <h2 className={styles.editorName} onClick={() => setEditingName(true)}>
              {list.name}
            </h2>
          )}
          <div className={styles.editorHeaderMeta}>
            <button className={styles.editorDateBtn} onClick={() => setEditingDate(true)} title="Change date" type="button">
              📅 {formatListDate(list.date)}
            </button>
            {list.mealPlan ? <span className={styles.editorMealPlanTag}>🍽 {list.mealPlan}</span> : null}
          </div>
        </div>
        <div className={styles.editorHeaderActions}>
          ...
          <button className={styles.btnTelegramDisabled} disabled title="Coming soon - send to Telegram" type="button">
            ✈ Send to Telegram
          </button>
          <button className={styles.btnDeleteList} onClick={() => setShowDeleteConfirm(true)} title="Delete list" type="button">
            🗑
          </button>
        </div>
      </div>
      ...
      <div className={styles.editorItems}>
        {list.items.map((item, index) => (
          <ItemRow
            dropPosition={dragOverState?.itemId === item.id ? dragOverState.position : null}
            index={index}
            item={item}
            key={item.id}
            ...
            onUpdate={(changes) => void onUpdateItem(list.id, item.id, changes)}
        ...
        <div className={styles.newListModal}>
          ...
          <div className={styles.newListFooter}>
            ...
            <button
              ...
              onClick={() => {
                if (dateValue) {
                  void onUpdateList(list.id, { date: new Date(`${dateValue}T12:00:00`).toISOString() });
                }
                setEditingDate(false);
              }}
              type="button"
    ...
    {showDeleteConfirm && (
      ...
      >
        <div className={styles.newListModal}>
          ...
          <div className={styles.newListBody}>
            <p>Are you sure you want to delete <strong>{list.name}</strong>? This cannot be undone.</p>
          </div>
          <div className={styles.newListFooter}>
            <button
              className={styles.btnGhost}
