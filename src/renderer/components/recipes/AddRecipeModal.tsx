...
function toCuisineValue(value: unknown): CuisineValue | "" {
  return typeof value === "string" &&
    CUISINE_VALUES.includes(value as CuisineValue)
    ? (value as CuisineValue)
    : "";
}

function toIngredientGroups(
  recipe?: RecipeEditorInput | null
): IngredientGroupDraft[] {
  const ingredients =
    recipe && Array.isArray(recipe.ingredients) ? recipe.ingredients : [];
  ...
  for (const value of ingredients) {
    const ingredient = isRecord(value)
    ...
    const ingredientId =
      typeof ingredient.id === "string" ? ingredient.id : createUuid();
...
export function AddRecipeModal({
  ...
  const lastFocusRequestKeyRef = useRef<number | undefined>(
    focusTitleRequestKey
  );
  ...
  useEffect(() => {
    ...
    if (openingNow) {
      setForm({
        ...
        prepTime:
          initialRecipe?.prepTime != null ? String(initialRecipe.prepTime) : "",
        cookTime:
          initialRecipe?.cookTime != null ? String(initialRecipe.cookTime) : "",
        difficulty: toEditorString(initialRecipe?.difficulty),
        cuisine: toCuisineValue(initialRecipe?.cuisine),
        rating:
          initialRecipe?.rating != null ? String(initialRecipe.rating) : "",
        cookNotes: toEditorString(initialRecipe?.cookNotes),
        instructions: payloadToInstructionDrafts(
          toEditorStrings(initialRecipe?.instructions)
        ),
  ...
  async function handleSave() {
    ...
    const input: CreateRecipeInput = {
      ...
      tags: form.tagsText
        ...
        .filter(Boolean),
      origin:
        initialRecipe?.origin === "imported" ? initialRecipe.origin : "manual",
      prepTime: form.prepTime.trim()
        ? Number.parseInt(form.prepTime.trim(), 10)
        : null,
      cookTime: form.cookTime.trim()
        ? Number.parseInt(form.cookTime.trim(), 10)
        : null,
      difficulty: form.difficulty.trim() || null,
      cuisine: toCuisineValue(form.cuisine) || null,
      rating: form.rating.trim()
        ? Number.parseInt(form.rating.trim(), 10)
        : null,
  ...
  return (
    ...
    >
      {flaggedIngredients.length > 0 ? (
        <div
          className="mb-4 rounded-card border border-orange/30 bg-orange/5 p-3"
          role="status"
        >
          <p className="font-semibold text-text">Review imported ingredients</p>
          <p className="mt-1 text-sm text-text-muted">
            These ingredients were parsed with lower confidence. You can edit
            them before saving.
          </p>
                ...
                {form.instructions.map((instruction, index) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <Button
                        className="h-7 min-h-8 w-7 min-w-8 p-0 text-xs opacity-0 transition group-hover:opacity-100 group-focus-within:opacity-100"
                        disabled={index === 0}
                        onClick={() => {
                          const next = [...form.instructions];
      ...
      <div className="mt-4 rounded-card border border-cream-dark bg-cream/70 p-3 sm:mt-5 sm:p-4">
        <div className="mb-2.5">
          <h3 className="font-serif text-lg font-semibold text-text sm:text-xl">
            Ingredients
          </h3>
        </div>
        ...
        <div className="space-y-3">
          {form.ingredientGroups.map((group, groupIndex) => {
            ...
            return (
              <div
                className="rounded-btn border border-cream-dark bg-white p-2.5 sm:p-3"
                key={group.id}
              >
                {showGroupHeader ? (
