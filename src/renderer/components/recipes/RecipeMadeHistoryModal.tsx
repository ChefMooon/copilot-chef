...
function clampPhotoZoom(nextZoom: number) {
  ...
  return Math.min(
    PHOTO_ZOOM_MAX,
    Math.max(PHOTO_ZOOM_MIN, Number(nextZoom.toFixed(2)))
  );
}
...
export function RecipeMadeHistoryModal({
  ...
  return (
    <>
      <ModalShell
        ariaLabel="Recipe made history"
        bodyClassName="flex min-h-0 flex-1 flex-col overflow-hidden p-0"
        className="max-h-[92vh] w-full max-w-3xl"
        closeLabel="Close recipe made history dialog"
        footerRight={
        ...
        subtitle={`${history?.madeCount ?? 0} time${(history?.madeCount ?? 0) === 1 ? "" : "s"} made${history?.lastMadeAt ? ` · Last made ${new Date(history.lastMadeAt).toLocaleDateString()}` : ""}`}
        title={recipeTitle}
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {isLoading ? (
            ...
            <p className="text-sm text-text-muted">
              No made entries in this date range.
            </p>
          ) : (
                      ...
                      <button
                        ...
                        onClick={() => {
                          setActivePhoto({
                            ...
                            alt:
                              entry.photoFileName ?? `${entry.mealName} photo`,
                          });
                          setPhotoZoom(1);
                        }}
                        type="button"
      ...
      {portalRoot && activePhoto
        ? createPortal(
            <div
              aria-label="Cooking history photo viewer backdrop"
              className="fixed inset-0 z-[1060] flex items-center justify-center bg-black/55 px-3 py-4 backdrop-blur-[2px] sm:px-6 sm:py-6"
              ...
              >
                ...
                >
                  <div>
                    ...
                    <p className="mt-0.5 text-sm text-text-muted">
                      Use zoom controls or scroll to inspect details.
                    </p>
                  </div>
                ...
                <div
                  ...
                  onWheel={(event) => {
                    ...
                    updatePhotoZoom(
                      photoZoom +
                        (event.deltaY < 0 ? PHOTO_ZOOM_STEP : -PHOTO_ZOOM_STEP)
                    );
                  }}
                >
                  <div
